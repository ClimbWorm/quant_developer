...
class Strategy(object):
    ...
    def __init__(self, zzg_num,ema_num, trailing_stop_num, open_threshold_multiplier, path):
        ...
        # 寻找计算概率的表
        self.df_D: pd.DataFrame = pd.DataFrame()
    ...
    def add_ema(self, df_backtesting, period=21):
        df_backtesting[f'ema_{period}'] = talib.EMA(df_backtesting.Close, timeperiod=period)#.shift(1) ema的计算是包括当前bar的
        ...
        for t in range(len(df_backtesting)):
            ...
            if time.strftime("%M") == "45":
                ...
                if input_data == 1:
                    ...
                    try:
                        avg_OHLC = np.mean([df_60min.Open.loc[t_-14:t_],df_60min.High.loc[t_-14:t_],df_60min.Low.loc[t_-14:t_],df_60min.Close.loc[t_-14:t_]],axis=0)
                ...
                if input_data == 1:
                    ...
                    try:
                        avg_OHLC = np.mean([df_60min.Open.loc[t_-14:t_-1],df_60min.High.loc[t_-14:t_-1],df_60min.Low.loc[t_-14:t_-1],df_60min.Close.loc[t_-14:t_-1]],axis=0)
                        data_series = avg_OHLC.tolist() + [np.mean([df_backtesting.Open.loc[t],df_backtesting.High.loc[t],df_backtesting.Low.loc[t],df_backtesting.Close.loc[t]])]
                        rsi_sub = talib.RSI(pd.Series(data_series))[14]
                    except Exception as e:
                        rsi_sub = np.nan
    ...
    #                 if 80 < self.df_backtesting.loc[i_].rsi_60_min < 100:
    #                     return True
    #             return False
    #         else:
    #             if (self.df_backtesting.loc[i_].HA_Close - self.df_backtesting.loc[i_].HA_Open) > 0:
    #                 if 0 < self.df_backtesting.loc[i_].rsi_60_min < 20:
    #                     return True
    #             return False
    #     else:  # 没有突破
    #         if open_direction_ == TradeDirection.SHORT:
    #             if (self.df_backtesting.loc[i_ - 1].HA_Close - self.df_backtesting.loc[i_ - 1].HA_Open) < 0:
    #                 if 80 < self.df_backtesting.loc[i_ - 1].rsi_60_min < 100:
    #                     return True
    #             return False
    #         else:
    #             if (self.df_backtesting.loc[i_ - 1].HA_Close - self.df_backtesting.loc[i_ - 1].HA_Open) > 0:
    #                 if 0 < self.df_backtesting.loc[i_ - 1].rsi_60_min < 20:
    #                     return True
    #             return False
    ...
    def exit_order(self,i_,CurrentTime_):
        ...
        for order in self.is_alive_orders:
            ...
            if order.open_direction == TradeDirection.SHORT:
                ...
                if (self.df_backtesting.loc[i_-1].Low < self.df_backtesting.loc[i_-1][f"ema_{self.ema_num}"]) & ( # Todo 这边的Low后续可以改成close试试
                        order.is_open_threshold == 0):  # 还未开启ema21的移动止损
                    order.stoploss_line = order.EntryPrice + 1

                if (self.df_backtesting.loc[i_-1].Low < order.open_ema_price_threshold) & (order.is_open_threshold == 0):
                    order.stoploss_line = self.df_backtesting.loc[i_-1][f"ema_{self.ema_num}"]
                    order.is_open_threshold = 1

                if order.is_open_threshold == 1:
                    order.stoploss_line = self.df_backtesting.loc[i_-1][f"ema_{self.ema_num}"]
            else:
                if (self.df_backtesting.loc[i_-1].High > self.df_backtesting.loc[i_-1][f"ema_{self.ema_num}"]) & (# Todo 这边的High后续可以改成close试试
                        order.is_open_threshold == 0):  # 还未开启ema21的移动止损
                    order.stoploss_line = order.EntryPrice - 1

                if (self.df_backtesting.loc[i_-1].High > order.open_ema_price_threshold) & (order.is_open_threshold == 0):
                    order.stoploss_line = self.df_backtesting.loc[i_-1][f"ema_{self.ema_num}"]
                    order.is_open_threshold = 1
    ...
    def main(self):
        ...
        # print(traverse_list)
        for i in traverse_list[:2000]:#range(3000,e):# 因为前面有些信息缺失2000
            CurrentTime = pd.to_datetime(self.df_backtesting.loc[i].Time) # Todo 如果df_backtesing是通过读入外部文件产生的，这边需要转化成时间形式，不然直接读就行
...
def save_result(zzg_num,ema_num,open_threshold_multiplier,save_result_path, csv_path):
    ...
    if os.path.exists(path_pic):
        ...
        strategy = Strategy(zzg_num, ema_num, 1, open_threshold_multiplier, csv_path)  # Todo 对于不需要获取最终结果的，或者没有用到trailing stop的，第二个参数写一个除0外的数就可以
        strategy.main()
...
if __name__ == '__main__':
    ...
    # Todo 保存路径稍后再改
    save_result_path = r'F:\pythonHistoricalTesting\pythonHistoricalTesting\code_generated_csv\ema21_trailing_stop\only_ha_ema_differ\use_low_high_stopline_out'
    # local_core_count:int = 10
    rmt_batch_id = None
    rmt_work_dir = None
    rmt_param_file_name = None
    rmt_core_count = None
    rmt_mode = False
    if len(sys.argv) > 1 and len(sys.argv) != 5:
        raise Exception('Unknown parameters, quiting...')
    if len(sys.argv) > 1:
        rmt_batch_id = sys.argv[1]
        rmt_core_count = sys.argv[2]
        rmt_param_file_name = sys.argv[3]
        rmt_work_dir = sys.argv[4]

        rmt_core_count = int(rmt_core_count)

        rmt_mode = True

    if rmt_mode:
        save_result_path = rmt_work_dir
        print(save_result_path)
        rmt_param_file = './' + rmt_param_file_name
        if not os.path.exists(rmt_param_file):
            raise Exception('Can not find parameter file!')
        else:
            os.system("touch " + rmt_batch_id)
        df_parsing_param = pd.read_csv(rmt_param_file)
        pool = multiprocessing.Pool(rmt_core_count)
        print("remote processing started!")
        for i in range(len(df_parsing_param)):
            line = df_parsing_param.iloc[i]
            _,zzg_num,ema_num,multiplier = line
            if i % 10 == 0:
                print(zzg_num,ema_num,multiplier)
            pool.apply_async(save_result,(zzg_num,ema_num,multiplier,save_result_path,rmt_work_dir + '/tools/backtesting_data/'))
            break
        pool.close()
        pool.join()
        ...
        # local_param_file = 
        # pool = multiprocessing.Pool(local_core_count)
        pass
