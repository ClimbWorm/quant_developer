...
hyper_parameter = [{"support_lens": support_lens,
                    ...
                    'dynamic_dev_for_flat': dynamic_dev_for_flat,  #
                   ...
                   for min1_bl_dev in numpy.arange(1.8, 2.251, 0.05)[2:]  # 布林带偏移
                   ...
                   for dynamic_dev_for_flat in numpy.arange(1.5, 2.5, 0.2)
...
class Strategy(StrategyTemplate):
    ...
    def body(self,
        ...
        dynamic_dev_for_flat: float = params.get('dynamic_dev_for_flat', )  #
        ...
        prior_dev_max = working_status.get('dev_max', -10)  # 波段最大dev
        prior_dev_min = working_status.get('dev_min', 10)  # 波段最小dev
        ...
        need_close: Direction = working_status.get('need_close', Direction.NONE)  # 需要平仓

        # 预判方向
        realtime_dev = curr_multi_bs.bs[main_len].realtime_dev  # 当前价格dev
        predict_direction: Direction = prior_main_direction
        dev_max: float = min(max(realtime_dev, prior_dev_max), 4.5)  # 防止突然太大
        dev_min: float = max(min(realtime_dev, prior_dev_min), -4.5)

        # 判断平仓
        if dev_max - dev_min > dynamic_dev_for_flat * min1_bl_dev:  # 到达门限
            if realtime_dev == dev_min:
                need_close = Direction.SHORT
                dev_max = -5
            else:
                need_close = Direction.LONG
                dev_min = 5
        ...
        if dev_min < -min1_bl_dev:
            need_open = Direction.LONG
        if need_open == Direction.LONG and dev_max > -0.5 * min1_bl_dev:
            need_open = Direction.NONE
        ...
        def batch_get_close_signal() -> OpenCloseParams:
            ...
            if OpenCloseTypes.DYNAMIC_DEV in close_types:
                ...
                if (need_close != Direction.NONE
                        and triggered  # 触发了
                        and need_close != curr_multi_bs.bs[8].price_mid  # 反向突破短期均线
                        and not curr_multi_bs.has_support_line(need_close)):
                    return OpenCloseParams(
                        direction=need_close,
                        type=OpenCloseTypes.DYNAMIC_DEV,
                        para=curr_multi_bs,
                        price=c_price, )
        ...
        working_status['dev_max'] = dev_max
        working_status['dev_min'] = dev_min
        working_status['need_close'] = need_close
        working_status['need_open'] = need_open
        if OpenCloseTypes.JUST_MA_TREND in open_types:
