...
class Strategy(StrategyTemplate):
    ...
    def __init__(self, name: str = None):
        StrategyTemplate.__init__(self, name or self.__class__.__name__)
        ...
        base_columns = ['datetime', 'Open', 'High', 'Low', 'Last',
                        'Volume', 'timestamp']

        self.data_sets: dict = {

            # 合成1分钟的数据及指标的配置
            'min1': BackTestingDataSetting(
                data_source='YMH21_1_SC',  # 数据源 配置文件名
                ...
                need_columns=base_columns,  # 策略需要的数据（列名）
        ...
        self.trade_params = {
            ...
            'take_profit': 5,
            'stop_loss': 6,
            'usd_per_tick': 5,  # 每个tick价值n美金  # todo 最好放symbol里
            'tick_size': 1,  # tick size
        }
        ...
        self.save_data_result = True,  # 保存回测数据结果
        self.show_data_result = True,  # 显示回测数据结果
        self.save_trade_result = True,  # 保存回测交易结果
        ...
        def _process(trade: SimuTrade) -> SimuTrade:
            ...
            def open_long() -> bool:
                ...
                for blt in bl_for_long:
                    if (prior_min1.Last < bolling['prior']['I'][blt] and
                            curr_min1.High > bolling['curr']['I'][blt] > curr_min1.Open):  # Last
            ...
            def open_short() -> bool:
                ...
                for blt in bl_for_short:
                    if (prior_min1.Last > bolling['prior']['I'][blt] and
                            curr_min1.Low < bolling['curr']['I'][blt] < curr_min1.Open):  # Last
            ...
            if trade.waiting_open:  # 开仓进行判断
                if open_long():
                    direction = Direction.LONG  # 开多
                elif open_short():
                    direction = Direction.SHORT  # 开空
                ...
                if direction != Direction.NONE:
                    trade.set_open(direction,
                                   curr_index,  # 当前bar
                                   curr_min1['Last'],  # 当前价
                                   amount=self.trade_params['once_lots'],  # 开仓量
