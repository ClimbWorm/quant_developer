...
pos_up_in = [{'min1_bolling_len': min1_bolling_len,
              'min1_bolling_dev': min1_bolling_dev,
              }
             for min1_bolling_len in range(15, 31, 5)
             for min1_bolling_dev in list(zip(numpy.arange(1.5, 2.51, 0.25),
                                              numpy.arange(-1.5, -2.51, -0.25)))
             ]

pos_down_in = [{'min1_bolling_len': min1_bolling_len,
                'min1_bolling_dev': min1_bolling_dev,
                }
               for min1_bolling_len in range(5, 21, 5)
               for min1_bolling_dev in list(zip(numpy.arange(-2.5, -1.7, 0.25),
                                                numpy.arange(2.5, 1.7, -0.25)))
               ]

oppo_up_in = [{'min1_bolling_len': min1_bolling_len,
              'min1_bolling_dev': min1_bolling_dev,
              }
             for min1_bolling_len in range(20, 31, 5)
             for min1_bolling_dev in list(zip(numpy.arange(-1, 0.1, 0.25),
                                              numpy.arange(1, -0.1, -0.25)))
             ]

oppo_down_in = [{'min1_bolling_len': min1_bolling_len,
              'min1_bolling_dev': min1_bolling_dev,
              }
             for min1_bolling_len in range(20, 31, 5)
             for min1_bolling_dev in list(zip(numpy.arange(0.5, 2.51, 0.5),
                                              numpy.arange(-0.5, -2.51, -0.5)))
             ]

total_in = pos_up_in + pos_down_in + oppo_up_in + oppo_down_in
...
hyper_parameter_base = [{"min1_ma_periods": min1_ma_periods,  # 一分钟线 4个周期
                   ...
                   for take_profit_trigger_p in numpy.arange(0.05, 0.5, 0.01)
                   ]

hyper_parameter = [dict(**x, **y)  # 取合并后的前值
                   for x in total_in
