...
_bl_dev = numpy.arange(-2.0, 2.1, 0.25)
min1_bl_dev = zip(_bl_dev, _bl_dev[::-1])  # 一分钟 开多 开空 布林带 偏差倍数
min5_bl_dev = numpy.arange(0.5, 2.1, 0.25)  # 布林带 偏差倍数
min1_ma_comb = [(a, b, c, d) for a in min1_mp_list for b in min1_mp_list
                for c in min1_mp_list for d in min1_mp_list if a < b < c < d]  # 均值周期选择范围组合
...
hyper_parameter = [{"min1_ma_periods": min1_ma_periods,  # 一分钟线 4个周期
                   ...
                   for min5_bolling_len in bolling_line_list[:]
                   for min5_tp_bolling_dev in min5_bl_dev
                   for min5_sl_bolling_dev in min5_bl_dev
...
class Strategy(StrategyTemplate):
    ...
    def body(self,
        ...
        def _process(trade: SimuTrade) -> SimuTrade:
            ...
            def close_long() -> Union[bool, Tuple[bool, str, Any]]:
                """平多判断"""
                # 止盈  5分钟布林带上轨外
                used_price_ = (min1.Last if decide_open_type == 'Close' else min1.High)
                if used_price_ > min5_bolling['tp_up']:
                    return (True, 'tp',  # 止盈标记
                            min1.Last if decide_open_type == 'Close' else min5_bolling['tp_up'])

                # 止损
                p_ = {'bar': trade.stop_loss,  # bar low
                      # 'boll_I': min1_bolling['down'],
                      'boll_II': min5_bolling['sl_down'],
                      }
                used_price_ = (min1.Last if decide_open_type == 'Close' else min1.Low)
                if used_price_ < p_[stop_loss_type]:
                    return (True, 'sl',  # 止损标记
                            min1.Last if decide_open_type == 'Close' else p_[stop_loss_type])
                else:
                    return False

            def close_short() -> Union[bool, Tuple[bool, str, Any]]:
                """平空判断"""
                # 止盈  5分钟布林带下轨外
                used_price_ = (min1.Last if decide_open_type == 'Close' else min1.Low)
                if used_price_ < min5_bolling['tp_down']:
                    return (True, 'tp',  # 止盈标记
                            min1.Last if decide_open_type == 'Close' else min5_bolling['tp_down'])

                # 止损
                p_ = {'bar': trade.stop_loss,  # bar high
                      # 'boll_I': min1_bolling['up'],
                      'boll_II': min5_bolling['sl_up'],
                      }
                used_price_ = (min1.Last if decide_open_type == 'Close' else min1.High)
                if used_price_ > p_[stop_loss_type]:
                    return (True, 'sl',  # 止损标记
            ...
            if trade.waiting_open:  # 开仓进行判断
                ...
                if direction in [Direction.CLOSE_LONG, Direction.CLOSE_SHORT]:
                    trade.set_close(close_bar=curr_index,  # 当前bar
                                    close_type=close_ret[1],
                                    close_price=close_ret[2],  #
                                    datetime_=min1['datetime'],  # 平仓时间
                                    )
                # 返回最后结果
                return trade
