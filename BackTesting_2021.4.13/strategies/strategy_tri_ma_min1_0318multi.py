...
pos_up_in = [{'min1_bl_len': min1_bl_len,
              'min1_bl_dev': min1_bl_dev,
              'in_ma_patten': 'positive_correlation',  # 开仓判断时 均线图形正相关还是负相关
              }
             for min1_bl_len in range(10, 31, 5)
             for min1_bl_dev in numpy.arange(1.0, 2.01, 0.2)
             ]

oppo_in = [{'min1_bl_len': min1_bl_len,
            'min1_bl_dev': min1_bl_dev,
            'in_ma_patten': 'negative_correlation',  # 开仓判断时 均线图形正相关还是负相关
            }
           for min1_bl_len in range(20, 31, 5)
           for min1_bl_dev in numpy.arange(-0.5, 1.51, 0.2)
           ]
total_in = pos_up_in + oppo_in  #

# 超参数
hyper_parameter_base = [{"min1_ma_periods": min1_ma_periods,  # 一分钟线 4个周期
                         'min1_ma_type': min1_ma_type,
                         'min1_bolling_ma_type': min1_bolling_ma_type,
                         # 进场
                         'forbidden_in_period': forbidden_in_period,  # 触发平仓类型
                        ...
                        # 进场
                        for forbidden_in_period in [(list(range(8, 17)) + [0, ]), ]  # 禁止开仓时间段
                        ...
                        for stop_loss_fixed_p in numpy.arange(0.1, 0.51, 0.05)
                        for take_profit_type in ['trigger', ]
                        for take_profit_trigger_p in numpy.arange(0.05, 0.251, 0.01)
                        ]
...
class Strategy(StrategyTemplate):
    ...
    @catch_except()
    def body(self,
             working_trades: List[SimuTrade],  # 要回测的交易们 (未开仓/已开仓未平仓）
        ...
        # 进场
        forbidden_in_period = params.get('forbidden_in_period', )  # 禁止开仓时间段
        ...
        # 动态调整
        stop_loss_fixed_p *= min1.avg_range_ratio
        take_profit_trigger_p *= min1.avg_range_ratio
        ...
        def multi_ma_patten(minute_x_ma: dict,  # 哪个分钟周期的均线
                            speed_up: bool = False,  # 是否加速，即均线按序发散
                            ) -> Direction:
            """多均线 形态 """
            if speed_up:
                pt = minute_x_ma['I'] > minute_x_ma['II'] > minute_x_ma['III']
            else:
                pt = (minute_x_ma['II'] > minute_x_ma['III']
        ...
        def calc_open_direction() -> OpenCloseParams:
            """判断开仓方向"""
            # 根据当前均线排列，分开考虑
            multi_ma_direction = multi_ma_patten(min1_ma, speed_up=False)
        ...
        def _process_open(trade_: SimuTrade):
            ...
            # 在禁止时间段，不开仓
            if min1.datetime.hour in forbidden_in_period:
                return trade_
